...
- [ ] **Fix Wake-up Stuck State (Deep Sleep)**
  - Resolve the boot-loop/lock-up issue when waking from deep sleep.
  - Correct the `sleep()` execution flow and implement wake-cause detection in `setup()`.
  - Detailed analysis: `docs/bug_wake_stuck.md`.
...
- [ ] **Power Management Refactor**
  ...
  - Optimize active and standby power consumption.
  - Strategy: `docs/power_optimization.md`.
- [ ] **Brownout Response**
